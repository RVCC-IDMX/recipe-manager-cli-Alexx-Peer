...
export async function promptForRecipeInfo() {
  ...
  const cookingTime = await number({
    message: 'Enter cooking time (number of minutes):',
    validate: function (value) {
      if (value <= 0) {
        return 'Cooking time must be greater than 0 minutes';
      }
      return true;
    }
  });
  ...
  const servings = await number({
    ...
    validate: function (value) {
      if (value <= 0) {
        return 'Serving size must be at least 1';
      }
      return true;
    }
  });
...
export async function promptForIngredient() {
  ...
  const unit = await input({
    ...
    validate: function (value) {
      if (value.trim() === '') {
        return 'Unit of measurement required';
      }
      return true;
    }
  });
...
export async function promptForStep() {
  ...
  const step = await input({
    message: 'Enter cooking step instruction:',
    validate: function (value) {
      if (value.trim() === '') {
        return 'Cooking step is required';
      }
      return true;
    }
  });
